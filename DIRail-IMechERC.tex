...
\node at (4,-2) {\includegraphics[width = 5cm]{InnotransStudis}};
     	...
     	\begin{column}[T]{7cm} 
			...
			\begin{tikzpicture}
                            ...
                            legend style = {at={(0.05,0.5)},
                              ...
                              \addplot[thick, draw = magenta!70!black] table[x=t,y=Fq, row sep=crcr] {WeibullEstCensored.dat}; 
                            
                             \addplot[thick, draw = orange!80!black] table[x=t,y=h, row sep=crcr] {WeibullEstCensored.dat}; 
                            
                             \addplot[thick, draw = red!70!black] table[x=t,y=Fqest, row sep=crcr] {WeibullEstCensored2.dat}; 
                                
                              
                              \addplot[thick, draw = green!70!black] table[x=t,y=Fqsim, row sep=crcr] {WeibullEstCensored.dat}; 
                              
                              
                             \addplot[thick, draw = blue!70!black] table[x=t,y=hest, row sep=crcr] {WeibullEstCensored.dat}
     ...
     \end{column}
     	\begin{column}[T]{8cm} 
         	\begin{center}
                    ...
                    draw = gray!50, 
                    fill =gray!50, opacity = 0.7, rounded corners, minimum height = 6.3cm, minimum width = 1.3cm},
                    modarr/.style={>=stealth, ->,line width = 3, draw = gray, opacity = 1},
                    workrect/.style={rectangle,align = center, 
                    draw = blue!90!black, 
                    fill =blue!90!black, opacity = 0.7, minimum width = 4.4cm, minimum height = 0.9cm, rounded corners},
                    otrect/.style={rectangle,align = center, 
                    draw = blue!90!black, 
                    fill =blue!90!black, opacity = 0.7, minimum width = 7.5cm, minimum height = 0.3cm, rounded corners},  
                    node distance = 1cm]
...
{
\usebackgroundtemplate{\includegraphics[width= \paperwidth]{EmmaDriver.jpg}}
...
\begin{columns}[t] 
     \begin{column}[T]{7cm} 
		...
		\item IoT-Connection
		\item Autonomes Fahren (geplant f\"ur 2018)
		\end{itemize}
...
\begin{columns}[t] 
     ...
     \end{column}
     	\begin{column}[T]{7cm} 
         	\begin{center}
		\vspace{-.5cm}
            		\href{https://www.youtube.com/watch?v=_z9fII6lbig}{\includegraphics[width=0.9\textwidth]{Team}}\\
			Team FH$_{2}$Rail\\
			IMechE Railway Challenge 2017\\ \vspace{.5cm}
			9. Platz von 11 Teilnehmern
        		\end{center}
     \end{column}
 \end{columns}
}

\frame{\frametitle{Welchen Vorteil haben Sponsoren?}
\framesubtitle{}
\begin{columns}[t] 
     \begin{column}[T]{7cm} 
     	\begin{itemize}
...
%		\end{itemize}
		...
		\item Warnwesten-Sponsor (800 \euro)
		\item Dinner-Sponsor (250 \euro)
		\end{itemize}
...
\begin{columns}[t] 
     ...
     \end{column}
     	\begin{column}[T]{8cm} 
			...
			\only<4>{\includegraphics[width=1\textwidth]{DSC_1439}}
...
{
\usebackgroundtemplate{\includegraphics[width= \paperwidth]{Winners.jpg}}
...
\vspace{-0.5cm}
\only<2>{\begin{center}
\Huge \textbf{\color{mint!75!black}Dabei hilft Ihr Engagement!}
\end{center}}
}
}

{
\usebackgroundtemplate{\transparent{0.4}\includegraphics[width= \paperwidth]{Lecker}}
